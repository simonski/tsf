...
Can we 10x an engineer? - The Software Factory (sf) is an approach to this question.
...
SF approach is to model the roles and processes we have now and automate them.  Applying this process across the software engineering stack is the experient this approach is attempting to prove.
...
An SDLC process is not cast in stone - it changes.  The method of change is based on the outcomes, objectives, learning, experimentation, prior experience.   We model this with

- a ticket tracking system
- the history of the work
- separate roles with their objectives

Once a piece of WORK is completed, a new LEARNING function can be applied to examine the output and measure the success of the outcome.  A new ROLE can be composed as the outcome and inserted into the SDLC process.  If the success is greater, then the role can be deemed to be functionally "better" that previously.  In this case, the SDLC process will have iterated by design from v1 to V2.

This is thematically similar to backpropagation however really we are talking about reading outcomes in tickets, measuring success and composing more markdown descriptions of roles or composed roles.

## Components

<< DIAGRAM >> 

Ticket System (1) - stores tickets, has NO brains, is only CRUD.  Crucially: WELL DEFINED AND SOLID.
  Backend
  Server
  API
  Website (kanban/visualisation of work)

Worker (1+) - wrapper around LLM.   Crucially: maintains ZERO state between tasks.
